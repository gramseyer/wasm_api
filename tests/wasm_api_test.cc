...
TEST_CASE("call external", "[wasm_api]")
{
	...
	{
		runtime -> template link_fn<&throw_runtime_error>("test", "external_call");
		REQUIRE_THROWS_AS(runtime -> template invoke<void>("call1"), wasm_api::UnrecoverableSystemError);
	}
	SECTION("host error")
	{
		runtime -> template link_fn<&throw_host_error>("test", "external_call");
		REQUIRE_THROWS_AS(runtime -> template invoke<void>("call1"), wasm_api::HostError);
	}
	...
	SECTION("wasm error")
	{
		REQUIRE_THROWS_AS(runtime -> template invoke<void>("unreachable"), wasm_api::HostError);
	}
